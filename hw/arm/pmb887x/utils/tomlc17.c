...
static void datum_init_location_info(toml_datum_t *parent, toml_datum_t *item, const char *key, const char *file) {
  if (parent && parent->loc.key.len > 0) {
    item->loc.key.len = parent->loc.key.len;
    item->loc.key.ptr = MALLOC(parent->loc.key.len + 1);
    memcpy((char *) item->loc.key.ptr, parent->loc.key.ptr, parent->loc.key.len + 1);
  }

  if (file) {
    size_t len = strlen(file);
    item->loc.file = MALLOC(len + 1);
    memcpy((char *) item->loc.file, file, len + 1);
  }

  if (key) {
    size_t key_len = strlen(key);
    if (item->loc.key.len == 0) {
      char *ptr = MALLOC(item->loc.key.len + key_len + 1);
      memcpy(ptr, key, key_len + 1);
      item->loc.key.ptr = ptr;
      item->loc.key.len += key_len;
    } else {
      char *ptr = REALLOC((char *) item->loc.key.ptr, (item->loc.key.len + key_len + 2));
      ptr[item->loc.key.len] = '.';
      memcpy(ptr + item->loc.key.len + 1, key, key_len + 1);
      item->loc.key.ptr = ptr;
      item->loc.key.len += key_len + 1;
    }
  }

  if (item->type == TOML_TABLE) {
    for (size_t i = 0; i < item->u.tab.size; i++) {
      datum_init_location_info(item, &item->u.tab.value[i], item->u.tab.key[i], file);
    }
  } else if (item->type == TOML_ARRAY) {
    char key_temp[1024];
    for (int32_t i = 0; i < item->u.arr.size; i++) {
      sprintf(key_temp, "[%d]", i);
      datum_init_location_info(item, &item->u.arr.elem[i], key_temp, file);
    }
  }
}
...
static void datum_free(toml_datum_t *datum) {
  ...
  if (datum->loc.file)
    FREE((void *) datum->loc.file);

  if (datum->loc.key.ptr)
    FREE((void *) datum->loc.key.ptr);
  ...
  *dst = mkdatum(src.type);
  ...
  case TOML_TABLE:
    for (int i = 0; i < src.u.tab.size; i++) {
      size_t key_len = strlen(src.u.tab.key[i]);
      char *newkey_str = pool_alloc(pool, key_len + 1);
      if (!newkey_str) {
        *reason = "out of memory";
        goto bail;
      }
      memcpy(newkey_str, src.u.tab.key[i], key_len + 1);
      span_t newkey = {newkey_str, src.u.tab.len[i]};
  ...
  default:
    *dst = src;
    dst->loc.file = NULL;
    dst->loc.key.ptr = NULL;
    break;
  }

  if (src.loc.file) {
    size_t len = strlen(src.loc.file);
    dst->loc.file = MALLOC(len + 1);
    memcpy((char *) dst->loc.file, src.loc.file, len + 1);
  }

  if (src.loc.key.ptr) {
    dst->loc.key.len = src.loc.key.len;
    dst->loc.key.ptr = MALLOC(src.loc.key.len + 1);
    memcpy((char *) dst->loc.key.ptr, src.loc.key.ptr, src.loc.key.len + 1);
  }
...
/**
 * Init datum loc field.
 * */
void toml_init_datum_location_info(toml_result_t *result, const char *file_name) {
  datum_init_location_info(NULL, &result->toptab, NULL, file_name);
}
