...
/*
 * wasm: mid-TB MMIO accounting (see io_open_clock_window(), cputlb.c).
 * Called from a load/store helper: if a virtual-timer deadline is crossed
 * the timers are synced here so the upcoming device callback sees their
...
void wasm_io_sync_timers(void) {
	...
	if (deadline > 0 && ticks >= deadline && !bql_locked()) {
		bql_lock();
		icount2_sync();
		bql_unlock();
	}
