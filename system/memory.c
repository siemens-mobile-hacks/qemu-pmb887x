...
/* True while the commit sequence generated by the current (or, when read
 * from the tcg listener, the just-finished) transaction went through the
...
static uint64_t romd_signature(void)
{
    ...
    if (romd_off_mrs) {
         ...
         * More than ARRAY_SIZE off-list MRs cannot be represented: two
         * different sets would then share a signature and a wrong view
         * variant could be adopted - refuse reuse instead. */
        if (romd_off_mrs->len > ARRAY_SIZE(ptrs)) {
            return 0;
        }
...
/* A topology generation bump makes every stashed variant unmatchable
 * (lookup requires s->gen == topo_gen): drop them instead of keeping up
 * to 16 dead generations - and their dispatch trees and MR references -
 * resident (and delaying hot-unplug finalization). */
static void romd_stash_drop_all(void)
{
    for (unsigned i = 0; i < ROMD_STASH_SLOTS; i++) {
        RomdStashSlot *s = &romd_stash[i];
...
static FlatView *generate_memory_topology(MemoryRegion *mr)
{
    ...
    /* sig 0 = "more off-list MRs than the signature can represent":
     * never adopt or stash such a view (a wrong variant could be
     * picked up); it is only used for this one commit. */
    if (view->romd_sig == 0) {
        view->topo_gen = 0;
    }
                               ...
                               EventNotifier *e)
{
    ...
    /*
     * ioeventfd_update_pending alone does not re-render the views or
